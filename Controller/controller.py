...
def remove(project):
    ...
    g.db.commit()
    #manual cascading till know friegn keys are setup correctly
    return pull_tasks(project)
...
def split(project):
    ...
    db.execute("INSERT INTO task(projid,name,description,stage,starttime,exptcomptime,actcomptime,contributor) VALUES (%d,'%s','%s',%d,'%s','%s','%s',%d)" % (project,name,desc,stage,start,ect,act,user))
